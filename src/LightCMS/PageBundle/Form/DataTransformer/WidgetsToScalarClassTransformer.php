...
class WidgetsToScalarClassTransformer implements DataTransformerInterface
{
    ...
    public function __construct(ObjectManager $objectManager, $row)
    {
    ...
    public function reverseTransform($scalars)
    {
        ...
        foreach ($scalars as $scalar) {
            ...
            $entityClassName = $scalar->getEntityClassName();

            if (is_null($entityClassName)) {
                $entity = new Row();
                $entity->setPage($this->page);
            } else {
                $identifierFieldName = $scalar->getIdentifierFieldName()[0];
